...
extension UIImageView{
    ...
    func setVideoThumnail(imageUrl: URL) {
            let placeholderImage = UIImage(named: "Profile-Pic-Icon (1)")
            ...
            self.kf.setImage(
                ...
                options: [
                    .transition(.fade(1)),
                    .cacheOriginalImage,
                    .onFailureImage(placeholderImage), // Show placeholder on image load failure
                    .processor(DownsamplingImageProcessor(size: CGSize(width: 100, height: 100))), // Adjust the size as needed
                    .scaleFactor(UIScreen.main.scale) // Adjust the scale factor as needed
                ])
        }
...
extension Collection {
    func unfoldSubSequences(limitedTo maxLength: Int) -> UnfoldSequence<SubSequence,Index> {
        Swift.sequence(state: startIndex) { start in
    ...
    func every(n: Int) -> UnfoldSequence<Element,Index> {
        Swift.sequence(state: startIndex) { index in
